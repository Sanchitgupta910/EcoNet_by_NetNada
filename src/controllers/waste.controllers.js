...
const addWaste = asyncHandler(async (req, res) => {
  ...
  //Publish the update to Redis- so any connected dashboard clients get real-time updates
  ...
  try {
    ...
    await redis.publish('waste‑updates', JSON.stringify({ branchId, payload }));
  } catch (err) {
    console.error('❌ Failed to publish waste update to Redis:', err);
  }
